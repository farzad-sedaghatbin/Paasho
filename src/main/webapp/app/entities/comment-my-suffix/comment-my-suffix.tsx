...
import { getSearchEntities, getEntities } from './comment-my-suffix.reducer';
import { ICommentMySuffix } from 'app/shared/model/comment-my-suffix.model';
// tslint:disable-next-line:no-unused-variable
import { APP_DATE_FORMAT, APP_LOCAL_DATE_FORMAT } from 'app/config/constants';

export interface ICommentMySuffixProps extends StateProps, DispatchProps, RouteComponentProps<{ url: string }> {}

export interface ICommentMySuffixState {
  search: string;
}

export class CommentMySuffix extends React.Component<ICommentMySuffixProps, ICommentMySuffixState> {
  state: ICommentMySuffixState = {
    search: ''
  };
  ...
  render() {
    ...
    return (
      <div>
        <h2 id="comment-my-suffix-heading">
          Comments
            ...
            <tbody>
              {commentList.map((comment, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>{comment.userId ? <Link to={`user-1-my-suffix/${comment.userId}`}>{comment.userId}</Link> : ''}</td>
