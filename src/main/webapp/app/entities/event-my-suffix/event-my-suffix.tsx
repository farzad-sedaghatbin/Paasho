...
import { getSearchEntities, getEntities, reset } from './event-my-suffix.reducer';
...
export interface IEventMySuffixState extends IPaginationBaseState {
  search: string;
}
              ...
              <tbody>
                {eventList.map((event, i) => (
                  <tr key={`entity-${i}`}>
                    ...
                    <td>{event.likes}</td>
                    <td>{event.creatorId ? <Link to={`user-1-my-suffix/${event.creatorId}`}>{event.creatorId}</Link> : ''}</td>
