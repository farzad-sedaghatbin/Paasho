...
export const getSearchEntities: ICrudSearchAction<IEventMySuffix> = (query, page, size, sort) => ({
  type: ACTION_TYPES.SEARCH_EVENTS,
  payload: axios.get<IEventMySuffix>(`${apiSearchUrl}?query=${query}${sort ? `&page=${page}&size=${size}&sort=${sort}` : ''}`)
});
