...
import { getSearchEntities, getEntities } from './factor-my-suffix.reducer';
import { IFactorMySuffix } from 'app/shared/model/factor-my-suffix.model';
// tslint:disable-next-line:no-unused-variable
import { APP_DATE_FORMAT, APP_LOCAL_DATE_FORMAT } from 'app/config/constants';

export interface IFactorMySuffixProps extends StateProps, DispatchProps, RouteComponentProps<{ url: string }> {}

export interface IFactorMySuffixState {
  search: string;
}

export class FactorMySuffix extends React.Component<IFactorMySuffixProps, IFactorMySuffixState> {
  state: IFactorMySuffixState = {
    search: ''
  };
  ...
  render() {
    ...
    return (
      <div>
        <h2 id="factor-my-suffix-heading">
          Factors
            ...
            <tbody>
              {factorList.map((factor, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>{factor.eventId ? <Link to={`event-my-suffix/${factor.eventId}`}>{factor.eventId}</Link> : ''}</td>
                  <td>{factor.userId ? <Link to={`user-1-my-suffix/${factor.userId}`}>{factor.userId}</Link> : ''}</td>
