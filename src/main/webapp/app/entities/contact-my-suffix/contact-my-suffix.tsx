...
import { getSearchEntities, getEntities } from './contact-my-suffix.reducer';
import { IContactMySuffix } from 'app/shared/model/contact-my-suffix.model';
// tslint:disable-next-line:no-unused-variable
import { APP_DATE_FORMAT, APP_LOCAL_DATE_FORMAT } from 'app/config/constants';

export interface IContactMySuffixProps extends StateProps, DispatchProps, RouteComponentProps<{ url: string }> {}

export interface IContactMySuffixState {
  search: string;
}

export class ContactMySuffix extends React.Component<IContactMySuffixProps, IContactMySuffixState> {
  state: IContactMySuffixState = {
    search: ''
  };
  ...
  render() {
    ...
    return (
      <div>
        <h2 id="contact-my-suffix-heading">
          Contacts
            ...
            <tbody>
              {contactList.map((contact, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>{contact.type}</td>
                  <td>{contact.value}</td>
                  <td>{contact.idId ? <Link to={`user-1-my-suffix/${contact.idId}`}>{contact.idId}</Link> : ''}</td>
