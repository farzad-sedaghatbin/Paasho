...
import { getSearchEntities, getEntities } from './media-paasho.reducer';
import { IMediaPaasho } from 'app/shared/model/media-paasho.model';
// tslint:disable-next-line:no-unused-variable
import { APP_DATE_FORMAT, APP_LOCAL_DATE_FORMAT } from 'app/config/constants';

export interface IMediaPaashoProps extends StateProps, DispatchProps, RouteComponentProps<{ url: string }> {}

export interface IMediaPaashoState {
  search: string;
}

export class MediaPaasho extends React.Component<IMediaPaashoProps, IMediaPaashoState> {
  state: IMediaPaashoState = {
    search: ''
  };
            ...
            <tbody>
              {mediaList.map((media, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>{media.path}</td>
                  <td>{media.type}</td>
                  <td>{media.eventId ? <Link to={`event-paasho/${media.eventId}`}>{media.eventId}</Link> : ''}</td>
