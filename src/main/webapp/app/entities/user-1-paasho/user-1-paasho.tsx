...
import { getSearchEntities, getEntities } from './user-1-paasho.reducer';
import { IUser1Paasho } from 'app/shared/model/user-1-paasho.model';
// tslint:disable-next-line:no-unused-variable
import { APP_DATE_FORMAT, APP_LOCAL_DATE_FORMAT } from 'app/config/constants';

export interface IUser1PaashoProps extends StateProps, DispatchProps, RouteComponentProps<{ url: string }> {}

export interface IUser1PaashoState {
  search: string;
}

export class User1Paasho extends React.Component<IUser1PaashoProps, IUser1PaashoState> {
  state: IUser1PaashoState = {
    search: ''
  };
            ...
            <tbody>
              {user1List.map((user1, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>
                    {user1.favorits
                      ? user1.favorits.map((val, j) => (
                          <span key={j}>
                            <Link to={`category-paasho/${val.id}`}>{val.id}</Link>
                  ...
                  <td>
                    {user1.notificationId ? <Link to={`notification-paasho/${user1.notificationId}`}>{user1.notificationId}</Link> : ''}
                  </td>
